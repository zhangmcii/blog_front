<template>
  <PageHeadBack>
    ...
    <!-- <el-image :src="compressedImages[0]?.src"> </el-image> -->
    <template #action>
...
export default {
  ...
  methods: {
    ...
    handleFileChange(file, fileList) {
      ...
      if (existingFile) {
        this.originalFiles = this.originalFiles.filter((f) => f.name !== file.name)
    ...
    async compressAndDisplayImages() {
      ...
      const uncompressedFiles = this.originalFiles.filter(
        (file) => !this.compressedImages.some((img) => img.name === file.name)
      )

      for (const rawFile of uncompressedFiles) {
    ...
    handleFileRemove(file, fileList) {
      // 从 originalFiles 中移除对应文件
      this.originalFiles = this.originalFiles.filter((f) => f.name !== file.name)

      // 从 compressedImages 中移除对应文件
      this.compressedImages = this.compressedImages.filter((img) => img.name !== file.name)
